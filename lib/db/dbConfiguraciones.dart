...
class DatabaseHelperConfiguraciones {
  ...
  Future<bool> getUsaRuta() async {
    ...
    final List<Map<String, dynamic>> maps =
        await db.query('Configuraciones', where: 'id = ?', whereArgs: [1]);
    if (maps.isNotEmpty) {
  ...
  Future<void> setConfiguracion(bool usaRuta, bool clientesFiltrados,
      bool usaApertura, int bodegaDescarga) async {
    final db = await dbProvider.database;
    await db.insert(
      'Configuraciones',
      {
  ...
  Future<void> insertConfiguracionSiEstaVacia() async {
    ...
    final List<Map<String, dynamic>> configuraciones =
        await db.query('Configuraciones');
    ...
    if (configuraciones.isEmpty) {
      ...
      print(
          'La tabla Configuraciones ya tiene datos. No se realizó ninguna inserción.');
    }
  }
}
