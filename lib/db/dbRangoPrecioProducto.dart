...
class DatabaseHelperRangoPrecioProducto {
  ...
  Future<List<RangoPrecioProducto>> getRangosByProductoBarras(
      int codigo) async {
    final db = await dbProvider.database;
...
INNER JOIN productos ON RangoPrecioProducto.CodProducto= productos.codigo
  ...
  Future<double> getPrecioByProductoYCantidad(
      int codigoProducto, int cantidad, double precioFinal) async {
    final db = await dbProvider.database;
 ...
 ''', [codigoProducto, cantidad]);
    ...
    return precioRango != 0 ? precioRango : 0;
  }

  Future<int> insertRangoPrecioProducto(
      RangoPrecioProducto rangoPrecioProducto) async {
    final db = await dbProvider
        .database; // Asume que tienes una referencia a la base de datos
